...
<<<<<<< HEAD
  const markView = async () => {
    if (!activeLesson?._id || !course) return;
    try {
      const { data } = await api.post(`/courses/${course._id}/lessons/${activeLesson._id}/view`);
      setActiveLesson((prev) => (prev ? { ...prev, views: data.views } : prev));
    } catch {
      // silent - non-critical, view count is not essential to the learning flow
    }
  };
